...
<template>
  <div class="container my-5 text-center" v-if="data.loaded">
    <!-- Title -->
    <div class="d-flex justify-content-between align-items-center mb-4 flex-column flex-md-row text-center text-md-start">
      <h3 class="fw-bold mb-3 mb-md-0">آموزش‌های پرمخاطب</h3>
    </div>
...
const selectParentCategory = async (cat) => {
  ...
  // Load popular courses for this parent category (includes all subcategories recursively)
...
const loadCourses = async (categoryId = null) => {
  ...
  try {
    ...
    console.log('⭐ Popular Courses API Request:', requestData);
    const response = await $api.post('/course/popular', requestData);
    console.log('⭐ Popular Courses API Response:', response.data);
